breed [ bone-marrow-cells bone-marrow-cell ]
breed [HSPCs HSPC]
...
globals[
  ...
  num_leukocytes
  num_HSPC
  num_pro-inflammatory-cytokines
  num_anti-inflammatory-cytokines
]
...
to setup
  clear-all
  reset-ticks
  set num_pathogens 500
  set num_leukocytes 500
  set num_pro-inflammatory-cytokines 500
  set num_anti-inflammatory-cytokines 500
  make-bone-marrow
  ;let world-area (2 * max-pxcor + 1) * (2 * max-pycor + 1)
  create-leukocytes num_leukocytes [
    random-position
    set color blue + 1.5
    set shape "circle"
    set size molecule-size
  ]
  create-pathogens num_pathogens [
    random-position
    set color black
    set shape "circle"
    set size molecule-size - 0.1
  ]
  create-pro-inflammatory-cytokines num_pro-inflammatory-cytokines [
    random-position
    set color green + 1
    set shape "circle"
    set size molecule-size
  ]
  create-anti-inflammatory-cytokines num_anti-inflammatory-cytokines [
    random-position
    set color orange + 2
    set shape "circle"
    set size molecule-size

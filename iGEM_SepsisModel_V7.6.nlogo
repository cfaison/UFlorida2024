...
breed [bcells bcell]
breed [activeLeukocytes activeLeukocyte]
...
;  ]
  ...
  ask pathogens[
    ...
    ask patch-here [
      ...
      set HSPC-density (HSPC-density - (pathogen-density * infection-intensity)* 0.4)
      if HSPC-density < 0[
        set HSPC-density 0
      ]
    ]
  ...
  ask patches[
    ...
    set pathogen-density (pathogen-density * (100 / (active-leukocyte-density + 1)))
  ...
  ask patches [
    ...
    set HSPC-density (HSPC-density + (pro-cytokine-density * 0.000000004))
    set active-leukocyte-density ((pathogen-density + count pathogens) * pro-cytokine-density * 0.000000004); * ((pro-cytokine-density + 1000) / (anti-cytokine-density + 1000))
    set HSPC-density (HSPC-density - (anti-cytokine-density * 0.000000004))
    set suppressive-leukocyte-density ((pathogen-density + count pathogens) * anti-cytokine-density * 0.000000004); * ((anti-cytokine-density + 1000) / (pro-cytokine-density + 1000))
    ;set pro-cytokine-density (pro-cytokine-density * ((active-leukocyte-density + 1) / (suppressive-leukocyte-density + 1)))
    ;set anti-cytokine-density (anti-cytokine-density * ((suppressive-leukocyte-density + 1) / (active-leukocyte-density + 1)))

    if mode = 0 [set pcolor scale-color brown HSPC-density -1000 (max-HSPC-density * 2)]
    if mode = 1 [set pcolor scale-color blue leukocyte-density -1000 (max-HSPC-density * 4)]
    if mode = 2 [set pcolor scale-color green lymphoid-density -1000 (max-HSPC-density * 5)]
    if mode = 3 [set pcolor scale-color violet myeloid-density -1000 (max-HSPC-density * 5)]
    if mode = 4 [set pcolor scale-color gray active-leukocyte-density -1000 (max-HSPC-density * 2)]
    if mode = 5 [set pcolor scale-color cyan suppressive-leukocyte-density -1000 (max-HSPC-density * 4)]
    if mode = 6 [set pcolor scale-color red pro-cytokine-density -1000 (max-HSPC-density * 6000000)]
    if mode = 7 [set pcolor scale-color blue anti-cytokine-density -1000 (max-HSPC-density * 4000000)]
  ]
  tick
end
...
to infect
  if time-since-infection = 0[
    let num random (infection-intensity * 10)
...
"" ""
PENS
"Pathogen" 1.0 0 -16777216 true "" "plot (sum[pathogen-density] of patches) + count pathogens"

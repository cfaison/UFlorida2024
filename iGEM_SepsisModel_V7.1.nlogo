...
to culture
  if sum [HSPC-density] of patches >= 0.99 * max-system-cellcount[
...
;  ]
  ...
  ask pathogens[
    move
    ask patch-here [
      set pathogen-density (pathogen-density + 1)
      set HSPC-density (HSPC-density - (pathogen-density * infection-intensity))
      if HSPC-density < 0[
        set HSPC-density 0
      ]
    ]
  ...
  ask patches[
    ...
    set active-leukocyte-density (leukocyte-density * ((pathogen-density) / (pathogen-density + 1)))
    set leukocyte-density (leukocyte-density - active-leukocyte-density)
    ;set suppressive-leukocyte-density (leukocyte-density / ((pathogen-density + 3)))
    set pathogen-density (pathogen-density - (0.01 * active-leukocyte-density))
    if pathogen-density < 0 [ set pathogen-density 0]
    if pathogen-density > 10 [
      sprout-pathogens 10 [
        set color black
        set shape "circle"
  ...
  ask patches [
    ...
    if mode = 1 [set pcolor scale-color blue differentiated-density -1000 (max-HSPC-density * 15)]
...
"Pathogens" 1.0 0 -16777216 true "" "plot (sum [pathogen-density] of patches) * 100000000"
